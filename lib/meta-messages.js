...
async function instagramAccountId() {
  ...
  try {
    const res = await graph(PAGE_ID, { fields: 'instagram_business_account{id}' }, USER_TOKEN)
...
export async function fetchConversations(platform = 'messenger', after = null, limit = 25) {
  ...
  if (platform === 'instagram') {
    // Endpointul contului de Instagram răspunde imediat — și, când ceva
    // lipsește, spune ce anume. Cel al paginii, în aceeași situație, nu
    // răspunde deloc: 20 de secunde de așteptare pentru nimic. Deci întrebăm
    // întâi contul, și abia dacă el nu poate, încercăm pagina.
    const igId = await instagramAccountId()
    if (igId) {
      try {
        res = await graph(`${igId}/conversations`, { platform, fields, limit, after }, token)
      } catch (err) {
        // Lipsă de capacitate sau de permisiune: nu are rost să mai insistăm
        // pe endpointul paginii, care oricum ar atârna.
        if (err.code === 3 || err.code === 200 || err.code === 10) throw err
      }
    }
  }
...
export async function diagnose() {
  ...
  const [pageRes, subsRes, messengerRes, instagramRes] = await Promise.allSettled([
    graph(PAGE_ID, { fields: 'name,instagram_business_account{id,username,name}' }, USER_TOKEN),
    token
 ...
 */
export function inboxLink(personId, platform = 'messenger') {
  if (!personId) return null
  const folder = platform === 'instagram' ? 'instagram' : 'all'
  return `https://business.facebook.com/latest/inbox/${folder}` +
    `?asset_id=${PAGE_ID}&selected_item_id=${encodeURIComponent(personId)}`
}
