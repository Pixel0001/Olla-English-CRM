...
export async function fetchConversations(platform = 'messenger', after = null, limit = 25) {
  ...
  const fields = 'id,updated_time,message_count,unread_count,snippet,participants'

  let res = await graph(`${PAGE_ID}/conversations`, { platform, fields, limit, after }, token)

  // Instagram: dacă pagina nu întoarce nimic, mai încercăm o dată pe contul
  // de Instagram — la unele conturi doar acolo apar conversațiile.
  if (platform === 'instagram' && (res.data || []).length === 0 && !after) {
    ...
    if (igId) {
      try {
        ...
        // păstrăm primul răspuns; eroarea de aici n-ar lămuri nimic în plus
      }
    }
  }
